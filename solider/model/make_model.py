    ...
    def __init__(self, num_classes, camera_num, view_num, cfg, factory, semantic_weight):
        ...
        if self.ID_LOSS_TYPE == 'arcface':
            ...
            if self.reduce_feat_dim:
                ...
                self.fcneck.apply(weights_init_xavier)
                self.in_planes = cfg.MODEL.FEAT_DIM
            self.classifier = nn.Linear(self.in_planes, self.num_classes, bias=False) # Classifier without BIAS (Back of tricks Number 5)
            self.classifier.apply(weights_init_classifier)

        # BatchNormalization Neck (Back of tricks Number 5)
        self.bottleneck = nn.BatchNorm1d(self.in_planes)
        self.bottleneck.bias.requires_grad_(False)
        self.bottleneck.apply(weights_init_kaiming)
